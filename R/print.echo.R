...
print.echo <- function(x, ...) {
  ...
  if(x$n_trees == 1) {
    ...
    A_med <- round(stats::quantile(x$A$point, probs=0.5), 2)
    B_med <- round(stats::quantile(x$B$point, probs=0.5), 2)
    C_med <- round(stats::quantile(x$C$point, probs=0.5), 2)
    
    A_LB <- round(stats::quantile(x$A$CI[1,], probs=0.5), 2)
    B_LB <- round(stats::quantile(x$B$CI[1,], probs=0.5), 2)
    C_LB <- round(stats::quantile(x$C$CI[1,], probs=0.5), 2)
    
    A_UB <- round(stats::quantile(x$A$CI[2,], probs=0.5), 2)
    B_UB <- round(stats::quantile(x$B$CI[2,], probs=0.5), 2)
    C_UB <- round(stats::quantile(x$C$CI[2,], probs=0.5), 2)
